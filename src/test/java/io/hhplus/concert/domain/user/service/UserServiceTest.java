...
public class UserServiceTest {
	...
	void 존재하지않은_유저가_포인트충전을_요청할경우_NotFoundException_예외발생() {
		...
		verify(userPointHistoryRepository, never()).save(amount, UserPointHistoryStatus.CHARGE, null); // 내역 생성 0번
	}
	@Test
	void 포인트충전금액이_양수가_아닌_0이하의_음수값일경우_InvalidValidationException_예외발생(){
		...
		verify(userRepository, never()).save(null); // 정보 업데이트 0번
		verify(userPointHistoryRepository, never()).save(invalidAmount, UserPointHistoryStatus.CHARGE, null); // 내역 생성 0번
	}
	@Test
	...
	void 포인트충전을_성공한다() {
		...
		UserPointHistory pointHistory = new UserPointHistory(chargeAmount, UserPointHistoryStatus.CHARGE, user);
		when(userPointHistoryRepository.save(chargeAmount, UserPointHistoryStatus.CHARGE, user)).thenReturn(pointHistory);
		...
		verify(userPointHistoryRepository,times(1)).save(chargeAmount, UserPointHistoryStatus.CHARGE, user); // 내역생성 1번
	}
	@Test
	void 존재하지않은_유저가_포인트사용을_요청할경우_NotFoundException_예외발생() {
		...
		assertEquals( NOT_EXIST_USER, ex.getMessage()); // 메시지 검증
		verify(userRepository, never()).save(null); // 정보 업데이트 0번
		verify(userPointHistoryRepository, never()).save(amount, UserPointHistoryStatus.USE, null); // 내역 생성 0번
	}
	@Test
	void 포인트사용금액이_양수가_아닌_0이하의_음수값일경우_InvalidValidationException_예외발생(){
		...
		assertEquals( AMOUNT_SHOULD_BE_POSITIVE_NUMBER, ex.getMessage()); // 메시지 검증
		verify(userRepository, never()).save(null); // 정보 업데이트 0번
		verify(userPointHistoryRepository, never()).save(invalidAmount, UserPointHistoryStatus.USE, user); // 내역 생성 0번
	}
	@Test
	...
	void 포인트사용을_성공한다() {
		...
		UserPointHistory pointHistory = new UserPointHistory(useAmount, UserPointHistoryStatus.USE, user);
		when(userPointHistoryRepository.save(useAmount, UserPointHistoryStatus.USE, user)).thenReturn(pointHistory);
		...
		verify(userRepository, times(1)).save(user); // 정보 업데이트 1번
		verify(userPointHistoryRepository,times(1)).save(useAmount, UserPointHistoryStatus.USE, user); // 내역생성 1번
	}
	@Test
